...
>SVN（Subversion）和git类似吗，是一个开源的版本控制软件，分为服务端和客户端，本文主要介绍客户端的svn命令使用方法。

## 1、检出(checkout)
通俗地讲，检出就是把服务器上的代码仓库拷贝一份到本地，并创建一个本地仓库，建立客户端和服务器端的联系。这样，本地仓库的修改可以提交到服务器，或者服务器的仓库也可以更新到本地。
...
- svn checkouthttp://localhost/test/testapp --username vosamo
...
【注】当在本地仓库新建一个文件时，这个新文件是不被track的，所以要用add命令把它添加到本地仓库，这样就可以被追踪到了。
...
## 5、更新文件update
更新文件就是把远程仓库更新到本地，做一个同步。本地仓库的代码可能不是最新的，在多人协作开发中尤其这样，别人可能随时提交新的代码，为了保持一致，最好每次都update一下。
...
## 6、删除文件delete
svn delete命令既可以删除本地仓库的文件，也可以删除远程仓库的文件。
...
## 15、恢复本地修改 
注意: 本命令只是在本地操作，表示放弃当前的修改，并把本地工作区的内容恢复为本地仓库中最新的版本，可以理解成word当中的撤销Ctrl+Z。但是它不会恢复被删除的目录。
