...
- 1. 首先，引导加载器把处理器从实模式切换到32位保护模式，因为只有在该模式下，软件才能访问物理地址空间中所有高于1MB的内存。你只要知道在保护模式下，分段地址（段地址:偏移地址对）到物理地址的转换有所不同，偏移地址是32位，而不再是16位。
- 2. 第二，引导加载器直接通过特殊的x86 I/O指令访问IDE磁盘设备寄存器来读取内核。
